...
public class Game {
	...
	public int execMain(String cpn, String itemaction, String ismgid, String linkid, Integer cpnType, String msgId,
		...
		try { // 可能要编码
			...
			if (!retMsg.equals("")) {
				...
				msgAdd(cpn, retMsg, serverId, linkid, msgId, cpnType, cpnSet, cpnGet, msgSet, costSet, ismgSet,
	...
	private void insertMt(String cpn, String userInput, String retMsg, String linkid) {
		...
		//
		// SmsMoMtHandle smsMtHandle = new SmsMoMtHandle(dbIp, dbMtName, dbUser,
		// dbPwd, dbPort);
		String mtCompany = "";
		SmsMoMtHandle smsMtHandle = new SmsMoMtHandle();
		try {
	...
	private void smsProcess(String cpn, String itemaction, String linkid, String msgId, String tableName) {
		...
		logger.debug("the company is:" + company.getCompany());
		...
		// logger.debug("total num is:" + moTotal);
		if (moTotal < moLimit) {
			if (isSend(company)) {
				checkRetMsg(company);
			}
	...
	public void checkRetMsg(Companys mtcompany) {
		this.retMsg = companyMtMsgManager.selectByCompanyAndServerId(this.company, mtcompany.getCompanytag())
				.getMtmsg();
	}
