...
Ext.onReady(function() {
			...
			var store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : 'tempMt.ered?reqCode=queryTemtMt'
								}),
			...
			store.on('beforeload', function() {
					...
					if(!Ext.isEmpty(addate)){
					          addate= addate.format('Y-m-d').toString();
					}
						this.baseParams = {
								...
								sendstate: Ext.getCmp('sendstate').getValue(),
								comprecstat: Ext.getCmp('comprecstat').getValue(),
								lostflag: Ext.getCmp('lostflag').getValue(),
								addate : addate
						};
					});
			...
			var grid = new Ext.grid.GridPanel({
						...
						renderTo : 'companyMtDiv',// 和JSP页面的DIV元素ID对应
						height : 500,
						// width:600,
						autoScroll : true,
			     ...
			     'render' : function() {
			      var tbar = new Ext.Toolbar({
			         ...
			         new Ext.form.ComboBox({
											id : 'sendstate',
											hiddenName : 'sendstateId',
											triggerAction : 'all',
											store : new Ext.data.SimpleStore({
														fields : ['value', 'text'],
														data : [['DELIVRD', '成功(DELIVRD)'], ['1', '失败']]
													}),
											displayField : 'text',
											valueField : 'value',
											mode : 'local',
			...
			//输出改变字体颜色渲染器
			function renderSendstate(value){
				if(value=='DELIVRD'){
