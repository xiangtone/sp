...
<%
	...
	{
		//server.delCpBilling(spBillingId);
	}
	//确定对帐单无误，更新对帐单状态
	else  if(type==4 && spBillingId > 0)
	{
		//把帐单更新为待CP审核
		//server.updateCpBillingStatus(spBillingId, 1);
	}
	//帮CP确认帐单无误，更新对帐单状态
	else  if(type==5 && spBillingId > 0)
	{
		//把帐单更新为CP已审核
		//server.updateCpBillingStatus(spBillingId, 2);
	}
	//撤回待CP审核的帐单
	else if(type==6 && spBillingId > 0)
	{
		//server.updateCpBillingStatus(spBillingId, 0);
	}
	...
	String[] statusData = {"待审核","SP对帐中","SP已对帐","已付款"};
...
<script type="text/javascript">
	...
	{
		%>
		cpList.push(new joSelOption(<%= spModel.getId() %>,1,'<%= spModel.getShortName() %>'));
		<%
	}
	%>
	...
	function delCpBilling(id)
	{
		...
		{
			window.location.href = "cpbilling.jsp?type=3&spBillingId=" + id + "&<%= Base64UTF.decode(query) %>";	
		}
	}
	...
	function reExportCpBilling(id)
	{
		...
		{
			window.location.href = "cpbilling.jsp?type=2&spBillingId=" + id + "&<%= Base64UTF.decode(query) %>";
		}
	}
	...
	function sendCpBillingToCp(id)
	{
		...
		{
			window.location.href = "cpbilling.jsp?type=4&spBillingId=" + id + "&<%= Base64UTF.decode(query) %>";
		}
	}
	...
	function confirmCpBillingForCp(id)
	{
		...
		{
			window.location.href = "cpbilling.jsp?type=5&spBillingId=" + id + "&<%= Base64UTF.decode(query) %>";
		}
	}
	...
	function reCallCpBillingFromCpStatus(id)
	{
		...
		{
			window.location.href = "cpbilling.jsp?type=6&spBillingId=" + id;
		}
	}
			...
			<tbody>
				...
				<tr>
					...
					<td style="text-align: left">
						<a href="cpbillingdetail.jsp?query=<%= query %>&spBillingId=<%= model.getId() %>" >详细</a>
