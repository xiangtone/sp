...
public class CpBankInfoDao {
	...
	{
		String sql="SELECT cpb.*,cp.user_id AS com_user_id,cp.full_name,cp.short_name FROM daily_config.tbl_bank_account cpb "
				+ "LEFT JOIN daily_config.tbl_cp cp ON cpb.cp_id=cp.id ORDER BY cpb.id DESC";
	...
	{
		String sql = "select " + Constant.CONSTANT_REPLACE_STRING + " from daily_config.tbl_bank_account cpb "
				+ "LEFT JOIN daily_config.tbl_cp cp ON cpb.cp_id=cp.id where 1=1";
	...
	{
		String sql = "SELECT cpb.*,cp.user_id AS com_user_id,cp.full_name,cp.short_name FROM daily_config.tbl_bank_account cpb "
				+ "LEFT JOIN daily_config.tbl_cp cp ON cpb.cp_id=cp.id where cpb.id="+id;
	...
	{
		String sql = "insert into daily_config.tbl_bank_account(cp_id,type,bank_name,user_name,account,branch,status) "
	...
	{
		String sql = "update daily_config.tbl_bank_account set cp_id = '"
	...
	public boolean deleteCpBank(int id){
		String sql="delete from daily_config.tbl_bank_account where id="+id;
		return new JdbcControl().execute(sql);
	}
	public Map<String, Integer> checkData(int cpId,int type,int status,int id){
		...
		map.put("flag", 0);
		String sql ="select count(*) as sm from daily_config.tbl_bank_account where cp_id="+cpId+" and type="+type+" and status="+status+" and id!="+id;
		JdbcControl control=new JdbcControl();
