...
public class jj45 : sdk_Request.Logical.APIRequestGet
{
    protected override sdk_Request.Model.SP_RESULT GetSpCmd()
    {
        ...
        var userAgentv = OrderInfo.extraParams;
        if (string.IsNullOrEmpty(userAgentv))
            userAgentv = getUAV(OrderInfo.imei);
        string url = "http://bysms.5151pay.com/GetDYSms.ashx?MchNo=" + OrderInfo.id + "&MobileType=YD" + "&Fee=" + PayModel.paycode + "&MchId=" + PayModel.channelid + "&CPParameter=" + PayModel.appid +
            "&IMSI=" + OrderInfo.imsi + "&IMEI=" + OrderInfo.imei + "&ClientIP=" + OrderInfo.clientIp + "&ua=" + HttpUtility.UrlEncode(userAgent) + "&video_ua=" + HttpUtility.UrlEncode(userAgentv)
        ...
        var jVal1 = jobj["ResultCode"];
            ...
            {
                var jMoSms = jobj["MoSms"];
                if (jMoSms.ToString().Length != null && jMoSms.ToString().Length != 1)
                {
                    OrderInfo.spLinkId = OrderInfo.id.ToString();
                    var sms = new sdk_Request.Model.SP_2SMS_Result();
                    sms.port = jMoSms[0]["PayChannel"].ToString();
                    sms.msg = jMoSms[0]["MoSmsMsg"].ToString();
                    sms.port2 = jMoSms[1]["PayChannel"].ToString();
                    sms.msg2 = jMoSms[1]["MoSmsMsg"].ToString();
                ...
                else
                {
                    ...
                    sms1.port = jMoSms[0]["PayChannel"].ToString();
                    sms1.msg = jMoSms[0]["MoSmsMsg"].ToString();
                    return sms1;
                }
